...
\begin{exercise}\label{exer:variable_sum}
	...
	\end{align*}
	where the covariance is null since $x$ and $y$ independent. The reader can also compute expected value and variance directly from the \emph{pdf} to become comfortable with these types of calculations.
\end{exercise}
...
\begin{exercise}
	Now you know the drill. We want to determine the \emph{pdf} $f_{R}(r)$ of the statistic:
	\[
		R=\frac{X}{Y}
	\]
	We set \sidenotemark:
...
\section{Functional Inferences}

\section{Point Estimations}

\section{Interval Estimations}
