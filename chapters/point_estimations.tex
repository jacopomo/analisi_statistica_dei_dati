...
we usually intend 
	...
	\]
where we explicitly write that $\hat{\theta}$ depends on the observations $x_0$ and that $\Var\left[\hat\theta\right]$ is a function of $\theta$ evaluated at $\hat{\theta}$.
...
\begin{example}
	...
	\begin{figure}[H]
		...
		\caption{Visualization of the pmf constructed as a counterexample to the statement that a consistent estimator must have asymptotically null variance. The mass function is defined in a way such that as $N$ increases the central peak approaches unity, while the two-point tails diminish asymptotically to $0$ but get further away.}
	\end{figure}
...
It's not guaranteed that these exist and are finite, but when they do they prove to be helpful statistics that we can readily calculate and update as we gather new data. If enough moments exist, this method allows us to easily build consistent estimators with known covariance and (for the central limit theorem) asymptotically normal. They are not necessarily unbiased.
...
If $N$ is fixed \emph{a priori}, the counts $\{k_i\}$ follow a multinomial distribution:
...
When $N$ is not fixed (for instance, when observations are collected over a fixed time interval), it becomes itself a random variable, following a Poisson distribution:
\[
...
\begin{example}
	A good estimator for the standard deviation of a Gaussian distributed random variable is the \emph{root-mean-square} (RMS), which, being the MLE, is asymptotically efficient:
	\[
	\hat\sigma \equiv s_N = \sqrt{\frac{1}{N}\sum_i (x_i - \bar{x})^2}
	\]
	where $\bar{x}$ is the sample mean.
	
	This is not the only possible estimator for the standard deviation. We could use, for example, the \emph{mean-absolute-deviation} (MAD):
	\[
	...
	Consider the case where we are not completely certain about the underlying pdf. For example, suppose the data is contaminated by a small fraction $f$ of observations with a larger standard deviation $\sigma_2$.
	We model the total pdf as a mixture of two Gaussians with the same mean $\mu$ but different standard deviations (take $\sigma_2 = 3\sigma_1$):
	\[
	...
	\begin{figure}[H]
		...
		\caption{Visualization of the three Normal curves, one with standard deviation $\sigma_1$, one with $3\sigma_1$, and the contaminated sum pdf.}
	\end{figure}
	We consider small values of $f$ (e.g.\ $f=0.05$), so most of the data follow the distribution with standard deviation $\sigma_1$. This does not seem like an extreme situation, only a mild contamination. Let us now look at the efficiency ratio $r$ for different values of $f$:
	\begin{center}
...
\end{example}\todo{I won't talk about the history of the topic and the Eddington-Fisher debate without proper sources. We could add the historical side of things later. Also, we should talk about and add in the scripts how we got those numbers in the table.}
...
Let's assume we are measuring something that has independent counts, and so we assume a Poisson distribution, where the best estimator\sidenote{The calculation for the MLE for the $N$ Poisson extractions is found in \ref{eq:poisson_mle}} for $\mu$ is $\hat\mu = \frac{\sum_i k_i}{N}$ 
...
\hrule
\begin{center}
	\textbf{Exercises at: \ref{exer:point_est}}
\end{center}
\hrule