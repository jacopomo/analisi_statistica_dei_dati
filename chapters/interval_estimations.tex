...
As usual, the Bayesian formulation brings many nice properties such as respecting the Likelihood principle, obtaining clear, specific, and intuitive results. However, it depends on how we choose the prior, which can greatly influence the type of result we obtain. Also, we need $\theta$ to be able to admit a pdf in the first place.
...
For any outcome $x\in\{0,1,2,\dots,20\}$ we can draw a horizontal line in any of the belts in figure \ref{fig:binomial_Neyman_belts} and find our interval estimation for $p$. As we have already touched upon briefly, it's important to stick to one belt (and therefore ordering algorithm) for all measured values rather than \emph{flip-flopping} between them. If we "artificially" make the interval smaller by choosing a particular intersection or combination of different ordering algorithms, it only means that we have lowered the coverage below our \emph{CL}, and so we lose the very property that defined our intervals in the first place. The choice of ordering should be determined by the goal of the analyst, like choosing Likelihood Ratio for the inclusion of the \emph{MLE} or lower ordering for the upper bound, and must be maintained for all values of $\theta$.


\begin{example}
\label{ex:LR_vs_p_ordering}
	Let's build the Neyman band for a Normally distributed $x$ with unknown parameter $\mu$ (let's say $\sigma=1$ for simplicity). We choose, for the sake of this example, to use probability-ordering, finding a two-sided interval for $\mu$ at a $95\% CL$.
	
	Specifically, what we want is to solve (\ref{eq:int_definition}), which in our case corresponds to:
	
	\[
	\int_{x_1}^{x_2} \frac{1}{\sqrt{2\pi}}e^{-\frac{\left(x-\mu\right)^2}{2}}\dd x \geq 0.95
	\]
	
	
	
	Having ordered using p-ordering, we know that our $x$-interval will be symmetric about the mean, and so we can apply a variable change to arrive to the following:
	
	\[
	\int_{z_1}^{-z_1} \frac{1}{\sqrt{2\pi}}e^{-\frac{z^2}{2}}\dd z \geq 0.95
	\]
	
	Having applied this change of variables, we have written the null mean unitary variance (standard) Normal distribution, whose integral is found tabled in Appendix \ref{tables:normal}. In this case, we find $z_1 = 1.96$, which means that the probability-ordered belt will always be $2*z_1$ wide about $\mu$ for all $\mu$. This belt can then be inverted to find the interval for $\mu$ at a certain $z$ (or, quite simply, at a certain $x$).
	
	\begin{figure}[H]
		\centering
		\input{images/neyman_gauss.pgf}
		\caption{Neyman belt for a Normally distributed $x$ with fixed unitary variance. Here, we have used probability ordering at $95\% CL$ and we note that an interval exists for all possible values of $x$.}
		\label{fig:neyman_gauss}
	\end{figure}
	
	As seen in \ref{fig:neyman_gauss}, the belt is of simple interpretation: having fixed the variance the interval estimate is centered on the measured value with a defined width, and such an interval exists for all measured $x$, meaning that $\mu$ is \emph{covered}. However, let us restrict ourselves in the particular case of $\mu>0$. In this case, if we simply use probability ordering again, it would be analogous to truncating the above belt, excluding all negative $\mu$ values.
	...
	\begin{figure}[H]
		...
		\caption{Neyman belts for a Normally distributed $x$ with fixed unitary variance and $\mu>0$. On the left, we have used probability ordering, and on the right Likelihood Ratio ordering, both at $95\% CL$. Plotted also is the line for an observation at $x=-2.5$}
	...
	It is instructive to note that if we were to draw a vertical line at $\mu=0.5$ in the LR-ordered belt, we would intersect it at two $x$ points that define the same interval shown in figure \ref{fig:lr_ordering}.
\end{example}

\begin{example}
	Let's build a Neyman belt analytically, in the fortunate case where the cumulative admits a clean, closed form. We will study the common case of an observable $x$ distributed following a uniform pdf $p(x;m) = \frac{1}{m}$ for $0\leq x \leq m$ and $0$ otherwise. We observe one $x$ and wish to build an interval estimate for $m$ with a $90\%$ \emph{CL}. 
	
	Let's start by finding a lower limit for $m$, which means using "lower ordering":
	
	\[
	\int_0^{x_{max}} \frac{1}{m}\dd x = CL
	\]
	
	Remember, we integrate with a fixed $m$, and we are doing nothing more than finding the $x$ for which the cumulative equals $CL$: $F_{m}(x_{max}) = CL$. We can easily solve to find:
	
	\[
	x_{max} = m\cdot CL
	\]
	
	This equation defines a Neyman belt, from which we easily see that the one-sided interval for $m$ (after measuring $x_0$) is: 
	
	\[
	m\geq \frac{x_0}{CL}
	\]
	
	This belt is seen on the left side of figure \ref{fig:uniform_belts}.
	
	Let's solve the same problem, this time using \emph{LR} ordering to obtain a two-sided interval for our parameter. We know that for a uniform distribution, as we have worked out in appendix \ref{sec:uniform} the \emph{MLE} for $m$ in the case of one observation is $m=x$ itself, and the \emph{LR} function can be written as:
	
	\[
	LR(x) = \frac{x}{m}
	\]
	
	To satisfy Likelihood Ratio ordering, we need to pick the x with the largest values of \emph{LR} first, and we see that the Likelihood ratio is monotonously increasing in $x$: we are using nothing other than upper ordering. In this case, our interval in $x$ will be $[x_{min}, m]$, chosen so that:
	
	\[
	\int_{x_{min}}^m \frac{1}{m}\dd x = CL
	\]
	
	To show that the above integral corresponds to upper ordering, we also show the following steps that bring it in the familiar form written in section \ref{subsec:ordering}:

	\[
	\int_{x_{min}}^m \frac{1}{m}\dd x = \underbrace{\int_0^m \frac{1}{m}\dd x}_{1} - \underbrace{\int_0^{x_{min}} \frac{1}{m}\dd x}_{F_{m}(x_{min})} = CL
	\]
	
	\[
	\int_{x_{min}}^m \frac{1}{m}\dd x = CL \iff F_{m}(x_{min}) = 1 - CL
	\]
	
	Either way one chooses to do it, solving the above integral, we get that
	
	\[
	x_{min} = (1-CL)\cdot m \quad x_{max} = m
	\]
	
	which, after a measurement $x_0$, translates to an interval of:
	
	\[
	x_0\leq m \leq \frac{x_0}{1-CL}
	\]
	
	In this case, upper ordering gives us the correct upper limit, and we impose the lower limit knowing that $m$ must be greater than the measured $x_0$.
	...
	The following figure shows the two different belts that we were able to construct analytically for this problem, and it's clear how inverting the $x$ limits as a function of $m$ brings us the interval for $m$.
		...
		\caption{Different Neyman belts for the interval estimate on $m$, the parameter for $U\left[0,m\right]$. On the left, lower ordering is shown and on the right Likelihood Ratio ordering, both with $90\%$ \emph{CL}. An example measurement of $x_0=1$ is shown, to highlight how the belt can be inverted to obtain a one or two sided interval estimate.}
		\label{fig:uniform_belts}
	\end{figure}
	
	In this specific case with $90\%$ \emph{CL}, we use the measurement $x_0=1$ as an example, which brings the intervals of:
		
		\input{tables/uniform_belts.tex}
		
	In this case, we are more interested in an upper limit for $m$ so Likelihood/upper ordering would bring more information on the parameter, as the observation of $x_0$ itself already provides a natural lower limit for the value of $m$. 
\end{example}
...
\begin{definition}[Pivotal quantity]
	Given a pdf $p(x;\theta)$, a function $f(x,\theta)$ is said to be a pivot if and only if $\forall \theta_1,\theta_2\in A$ we have that:
	
	\[
	p\left(f(x,\theta_1);\theta_1\right) = p\left(f(x,\theta_2);\theta_2\right) \equiv p_f(f)
	\]
\end{definition}
...
Well, now to write out $p_z(z)$, we need to use the change of variable rule that we worked out way back in definition \ref{def:pdf_var_change}, essentially substituting and multiplying by the Jacobian of the inverse transformation:

	\[
	p_z(z) = \N(x(z))\left|\frac{\dd x}{\dd z}\right| = \frac{1}{\sqrt{2\pi}}e^{-\frac{z^2}{2}}
	\]

We see that the above expression is nothing more than a "standard Gaussian", which is a Normal distribution with null mean and unitary variance. This is independent of our original parameters, and all of the information we could ever hope to have on this distribution can be found on tables and in literature, as it is an extensively researched function. For example, appendix \ref{tables:normal} contains the integrals of the tails of this standard Gaussian, and some other helpful tables built off of pivotal quantities can be found at the end of these notes. The important thing is that $z$ itself is a function of the observable $x$ and the parameters $\mu,\sigma$, though its pdf is standard, and does not depend on these unknowns. I can easily, through the use of tables, make affirmations on $z$, which I can then invert to make affirmations on $\mu, \sigma$.

A possible strategy for when we have to deal with Normal distributions could then be to transform them through the pivotal quantity $z$, solve whatever problem it is that we are looking to solve (for example, but not limited to, finding the value of the cumulative at a certain point), and then transform $z$ back to obtain the necessary information on our parameters. 

Let's look into a key property of pivotal quantities that is in line with what we have been talking about for the last few pages. Let's consider the case of an interval estimate, where we have \emph{coverage}:
	
\begin{align*}
	\mathcal{C} &= \int_{o_{\theta}(x)\geq q(CL)} p(x;\theta)\dd x \\
	&= \int_{o_{\theta}(f)\geq c(CL)} p_f\left(f(x,\theta);\theta\right)\dd f = \int_{o(f)\geq c(CL)} p(f) \dd f
\end{align*}

We have written the known property that $\int p(x) dx = \int p(y) dy$, and in the last step used the fact that $f$ is a pivotal quantity and therefore its pdf is independent of the parameter.

What does this mean? If we express our density function through the pivotal quantity then the coverage is constant in the parameter. That means that the confidence region is easily found for all $\theta$ as the solution to $o(f(x,\theta))\geq c(CL)$. The pesky problem that we had, where we had to solve the integral for every possible fixed value of the parameter is now gone: once I solve it for one $\theta$, I have the full belt with constant coverage, pretty cool! 
...
\begin{theorem}[Likelihood Ratio (Wilks') theorem]
	...
	\begin{equation}
	\lambda\left(x,\theta\right) = -2\ln\left[\frac{p(x;\theta)}{p(x;\hat{\theta}_{MLE})}\right]
...
This easy-to-build quantity is a universal pivot for whichever pdf we start with, and we know its asymptotic distribution $p(\lambda)$ is a $\chi^2_\nu$, which means that:
...
Asymptotically in the \emph{degrees of freedom}, this distribution approaches a Gaussian with mean $\nu$ and variance $2\nu$. 

The proof of Wilks' theorem will not be discussed in these notes, though those who are interested can take a look at the following example to see how we conclude that $\lambda$ is distributed like a $\chi^2$, and how it approaches a Gaussian for high degrees of freedom:

\subsubsection{Derivation of the asymptotic distribution}
Let's derive the asymptotic distribution of the $LLR$, using the fact that Wilks' theorem guarantees its generality. Therefore, we are free to start with whatever distribution we wish, and we decide to use $N$ samples of a Normally distributed variable with known variance $\sigma^2$. We write the log Likelihood:
	
	\begin{align*}
		\lambda(x,\mu) &= 2\ln\overbrace{\left(Ce^{-\sum_i\frac{(x_i-\bar{x})^2}{2\sigma^2}}\right)}^{p(x;\hat{\mu}_{MLE})} - 2\ln\overbrace{\left(C e^{-\sum_i \frac{(x_i-\mu)^2}{2\sigma^2}}\right)}^{p(x;\mu)}\\
...
\begin{example}\label{ex:llr_poissonian}
	Let's try to calculate the confidence intervals for the mean $\mu$ of a Poisson distributed observable, $n$, using the LLR.
	...
	\]
	and, from \ref{eq:poisson_mle}, the maximum likelihood estimator for a single measurement is
	\[
	\hat\mu = n
	\]
	...
	\begin{align*}
		\lambda(n, \mu)
		&= -2 \ln \left(\frac{p\left(n;\mu\right)}{p\left(n;\hat\mu\right)}\right) \\
...
\hrule
\begin{center}
	\textbf{Exercises at: \ref{exer:interval_est}}
\end{center}
\hrule