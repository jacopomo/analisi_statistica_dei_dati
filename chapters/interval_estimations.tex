...
\begin{example}
	If we take a second, we can even find some edge case examples where the point estimate method breaks down, considering a Poisson \emph{pdf}. We know that the MLE is $\hat\mu=k$ with null bias and $\Var\left[\hat\mu\right] = \mu$. Therefore if I measure one count $k=1$ then I obtain the point estimate for $\mu = 1\pm1$. Only armed with this we would be tempted to say that we can't exclude $\mu\leq0$, even though we know that the counts can, by definition, only be positive.
...
\begin{definition}[Interval estimator]
	Given a \emph{pdf} $p(x;\theta)$ dependent on an unknown parameter $\theta\in A$, an \emph{interval estimator} is a statistic $f(x): X\rightarrowtail \mathcal{P}(A)$ which outputs a subset of the parameter space.
\end{definition}
...
\end{equation}
where $0 < c < 1$ is a constant called the \emph{credibility}. The set $f(x)$ represents a \emph{credible region}, i.e. a set of plausible values for $\theta$ such that the posterior probability of $\theta$ lying within it is equal to $c$:
\[
...
As usual, the Bayesian formulation brings many nice properties such as respecting the Likelihood principle, obtaining clear, specific, and intuitive results. However, it depends on how we choose the prior, which can greatly influence the type of result we obtain. Also, we need $\theta$ to be able to admit a \emph{pdf} in the first place. \todo{Here we can talk about Jeffrey's prior, if it ever comes up, or reference priors.}

Integral (\ref{eq:Bayes_int_eq}) admits infinite solutions, so we must introduce a criteria to be able to select the specific solution we want. The most agreed upon way to proceed is by selecting an \emph{ordering function}\sidenote{We will dedicate more time to ordering functions in the near future} $o(\theta)$ that assigns a priority hierarchy to the values of the parameter to include. 
...
Where $q(c)$ is a constant that depends on the credibility $c$ in a way such as to satisfy the equation. Usually, this allows us to find one unique solution, given $c$ and the ordering function. Essentially, we are finding an interval in $\theta$ where $o(\theta)\geq q_c$ and the above integral is equal to $c$.

The most common and simple choice is to use \emph{posterior ordering} $o(\theta) =\pi_x(\theta)$, which outputs credible region with the smallest measure in $\dd\theta$. This is desirable, as it intuitively restricts the credible values of the parameters more than any other ordering function. We note that this is not invariant for variable changes in the parameter space. 


\begin{example}
	Let's formulate an example: let's imagine to measure an observable that is distributed following a Poisson \emph{pdf} depending on a parameter $\mu$: $p(k;\mu) = \frac{e^{-\mu}\mu^k}{k!}$. However, let's assume that for some reason we know that $\mu$ admits a \emph{pdf} and that it is uniformly distributed between $0$ and $10$. 
	...
	We want to find, given a measurement $k$, an interval for $\mu$ with a credibility of $90\%$. We know this to mean that we want to find an interval in $\mu$ of the type $\mu\in[a,b]$ such that $\int_a^bp(\mu)\dd \mu= 0.9$. We can imagine there to be an infinite number of such intervals, and so we define different \emph{ordering functions} that change the type of interval we get.
...
We can see the power of this Bayesian case, and it's not hard to understand what an interval estimate looks like and means under this formulation. However, it is not always applicable, as the unknown parameters must admit \emph{pdf's} themselves.
...
In the general case, in order to find an interval estimate for a certain parameter $\theta$ we will have to solve the following equation:
...
If we think about it, on the left side we have an integral in $\dd x$, and on the right a constant that we fix beforehand, namely the "confidence level" \emph{CL}\sidenote{Don't worry about these definitions, they will all get cleared up in the near future. This part aims to provide a general outlook on the math that we will be using, so just bear with us}. Fundamentally, the equation that describes the interval in $\theta$ is, justifiably so, an inequality dependent on the parameter that we must solve. Let's never forget this as it will always be the final step. 

Now, what exactly is this $f(\theta)$? Well, since we are not in the Bayesian case and cannot directly say that the parameter admits a \emph{pdf}, we can only talk about the distribution of the observable: we want that, with probability \emph{CL}, the interval defined by $o_{\theta}(x)$ contains $\theta$\todo{SOS: re-state better}. As we will see, infinite such intervals exist for each fixed $\theta$ (there are infinite ways to integrate a normalized function such that its integral is equal to, say, $0.95$), and $o_{\theta}(x)$ serves as an "ordering function" with the aim of unambiguously defining one such interval by describing which $x$'s to include. 

This integral in eq. \ref{eq:int_definition} is nothing more than the integral of the \emph{pdf} itself, which we know to be the cumulative of the density function\sidenote{\emph{CDF's} start the integration from $0$, as the lower bound is not necessarily $0$ this integral is, at most, the difference of two \emph{CDF's}}, calculated on upper and lower bounds that may, in general, depend on the parameter. 

What this all means is that having fixed an observed $x_0$ (coming from \emph{pdf} $p(x;\theta)$), the ordering function describes a $\theta$-dependent interval for which the integral of the \emph{pdf} is greater than or equal to the \emph{CL}. Therefore, we can solve this expression (which we so naïvely wrote as $f(\theta) - CL \geq 0$) to find an interval in $\theta$, and that will be the estimate that we are looking for. 

Solving this equation is rarely simple, and can be done when the \emph{pdf} assumes a closed form cumulative, or at least when the latter is tabled\todo{such as the ones in ref.}. The next sections will break down each part of the process, using examples and visual aids to try to better explain this difficult, yet fundamental concept.

\subsection{Confidence levels and coverage}
Let's begin our piece-by-piece breakdown of eq. \ref{eq:int_definition} by being more precise in our definition of \emph{confidence level}:

\begin{definition}[Confidence level]
	Given a \emph{pdf} $p(x;\theta)$ dependent on an unknown parameter $\theta\in A$, and an interval estimator $f(x): X\rightarrowtail \mathcal{P}(A)$, we define the \emph{confidence level} (\emph{CL}) of $f(x)$:
...
Basically, we are saying that $f(x)$ has a confidence level \emph{CL} if it outputs an interval that contains the real value of $\theta$ with probability at least \emph{CL} - independently from the real value of $\theta$.
...
From this whole part we want to take away one very important concept: when we obtain an interval $[a,b]$ with confidence level \emph{CL}, that does not mean that the parameter lies within that interval with probability \emph{CL}. What it means, instead, is that the interval was generated by a procedure whose repeated use contains the true parameter with probability \emph{CL}.
...
The final ordering algorithm we will explore for now is one based on the \textbf{Likelihood Ratio} (some texts may call it "Feldman-Cousins ordering"). The idea is to use the Likelihood Ratio as the ordering function
	\[
	LR(x) = \frac{p(x|\theta)}{p(x|\hat{\theta}_{MLE})} = o_{\theta}(x)
	\]
which means, in practice, to solve the following system of equations:
	\[
	\begin{cases}
...
Analytically, the goal would be to define a function, or set of functions, of the type $x_{max}(\theta)$, $x_{min}(\theta)$, or their inverses. Sometimes, based on the ordering algorithm, \emph{pdf}, and parameter we are trying to estimate, we are able to build the whole belt analytically and define an interval for $\theta$ for all observed values of $x$. 


\begin{example}
	Let's build a Neyman belt analytically! We will study the common case of an observable $x$ distributed following a uniform \emph{pdf} $p(x;m) = \frac{1}{m}$ for $0\leq x \leq m$ and $0$ outside of that support. We observe one $x$ and wish to build an interval estimate for $m$ with a $90\%$ \emph{CL}. 
...
\begin{definition}[Pivotal quantity]
	Given a \emph{pdf} $p(x;\theta)$, a function $s(x,\theta)$ is said to be a \emph{pivot} if and only if $\forall \theta_1,\theta_2\in A$ we have that:
...
\begin{theorem}[Likelihood Ratio (Wilks') theorem]
	Let $p(x;\theta)$ be a \emph{pdf} with domain independent from $\theta\in \R^N$, and at least twice differentiable. In this case, the quantity:
...
This easy to build quantity is a universal pivot for whichever \emph{pdf} we start with, and we know its distribution $p(\lambda)$ is a $\chi^2_\nu$, which means that:
...
Let's start by writing $\chi^2_1$, which we get through squaring a Normally distributed $x$ and finding the \emph{pdf} of this new variable:
...
We use the inverse transformation to calculate the \emph{pdf} of $\chi_\nu^2$:
...
\begin{example}\label{ex:llr_poissonian}
	...
	This behavior can be quantified through the coverage function (see \ref{eq:coverage}):
	\[
