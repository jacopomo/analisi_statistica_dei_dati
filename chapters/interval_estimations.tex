...
We then determine a subset $f(x)$ of the parameter space\sidenote{Yes, $f(x)$ is a subset of the parameter space, at a fixed $x$} (typically an interval), depending on the observed data $x$, such that
\begin{equation}
...
\begin{example}
	...
	For example, if we choose to integrate over the zone where $p(\mu)\geq q_c$, we define two bounds (the two solutions $a,b$ of $p(\mu)\geq q_c$) such that 
	
	\[
	\int_{p(\mu)\geq q_c}p(\mu)\dd \mu = 0.9
	\]
