...
Now, what exactly is this $g(\theta)$? Well, since we are not in the Bayesian case and cannot directly say that the parameter admits a pdf, we can only talk about the distribution of the observable: we want that, with probability \emph{CL}, the interval defined by $o_{\theta}(x)$ contains $\theta$\todo{SOS: re-state better}. As we will see, infinite such intervals exist for each fixed $\theta$ (there are infinite ways to integrate a normalized function such that its integral is equal to, say, $0.95$), and $o_{\theta}(x)$ serves as an "ordering function" with the aim of unambiguously defining one such interval by describing which $x$'s to include. 
...
What this all means is that having fixed an observed $x_0$ (coming from pdf $p(x;\theta)$), \textbf{the ordering function describes a $\theta$-dependent $x$-interval for which the integral of the pdf is greater than or equal to the \emph{CL}}. Therefore, we can solve this expression (which we so naïvely wrote as $g_{x_0}(\theta) - CL \geq 0$) to find an interval in $\theta$, and that will be the estimate that we are looking for. 
...
\begin{definition}[Confidence level]
	Given a pdf $p(x;\theta)$ dependent on an unknown parameter $\theta\in A$, and an interval estimator $f(x): X\rightarrowtail \mathcal{P}(A)$, we define the \emph{confidence level} (\emph{CL}) of $f(x)$:
