...
So far we have talked about the basics of Probability, and introduced key concepts like probability mass functions. While these abstractions are extremely useful and will follow us through the course of our lives as physicists, realistically we will rarely measure an observable extracted from a completely known \emph{pdf}. Actually, if the probability density function is completely known, measuring the observable brings us no new information other than the measure itself - we learn nothing new about the world. 

Things start to get interesting when the distribution itself depends on some unknown parameter\sidenote{This parameter can be anything, it does not have to be observable, follow a \emph{pdf}, be a random variable, it can be anything that the density function of the observable depends on.}, and measuring the observable brings us new information on what that parameter may be. In this section we will write
	\[
	p_X(x;\theta)
	\]
to denote the \emph{pdf} of observable $x\in X$ dependent on some parameter $\theta\in M$, using the semicolon to distinguish observables from parameters. Our goal is that of the \emph{functional inference}, finding a \emph{function} that depends on the observables (statistic) that assigns a sort of "score" to every value of the parameters, indicating their plausibility. In short:
	\[
...
\begin{enumerate}
	...
	\item When transforming variables, the Likelihood transforms like the \emph{pdf} (so with $||J(x)||$), and therefore the ratio $\frac{L\left(\theta_1\right)}{L\left(\theta_2\right)}$ of two Likelihoods remains constant.
...
\begin{definition}[Sufficient statistic]
	Let $p(x;\theta)$ be the \emph{pdf} of $x$, depending on parameter $\theta$. Let $s(x)$ be a statistic (with dimensionality less than or equal to that of $x$) such that the probability of $x$ \emph{given} $s$ does not depend on $\theta$:
	\[
...
\begin{definition}[Ancillary statistic]
	The less used complementary cousin of the sufficient statistic: if $s(x)$ is a statistic such that its \emph{pdf} is not dependent on $\theta$:
	\[
...
\begin{theorem}[Darmois theorem]
	...
	In this case, the \emph{pdf} can be written as:
	\[
...
Basically, what this theorem is saying is that if I can write my \emph{pdf} in exponential form, and seperate it into a sum terms as shown in \ref{th:darmois} then I can identify the sufficient statistic for $p$. This is great and will be used when possible, however it's easy to imagine that the class of functions that can be written as exponentials is not as large as we'd like.
...
We note that, if the domain of $x$ is independent of $\theta$ and we have sufficient regularity in $p$ then we can commute the derivative with the integral and we get
...
This expression makes it clear that the Fisher Information is linear in $\ln L$, and since the Likelihood is multiplicative in the number of observations, the Fisher Information is additive (under the usual assumptions). Since we want the Information to represent how much the observations tell us about the parameter $\theta$, this is a property that we happily accept. 
