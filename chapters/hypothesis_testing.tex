...
\caption{Probability of the Test statistic considering null and alternative hypotheses. The critical region is separated from the acceptance region by a vertical line, and $\alpha, \beta$ are shown shaded. This Test corresponds to a right-tailed test statistic.}
...
\begin{theorem}[Neyman-Pearson]
	Given two simple hypotheses $H_0,H_1$, corresponding to the the same pdf but with different fixed values of $\theta$ $p(x;\theta_0),p(x;\theta_1)$, and let $T_{NP}$ be a Test based on the Likelihood ratio Test statistic
...
\sidenotetext{This is a left-tailed test, differently than what we have seen prior to this point}
