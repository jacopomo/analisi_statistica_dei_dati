...
\begin{figure}[H]
	\centering
	\input{images/hypothesis_testing_power.pgf}
	\caption{Power of a test as the alternative hypothesis $i$ changes. The same values as in image \ref{fig:hypothesis_testing_errors} have been used, and so $i=0$ corresponds to the hypothesis $H_0$, and $i=1$ corresponds to $H_1$ seen in the previous figure. As the alternative hypothesis changes (gets "further"), the power increases asymptotically towards $1$. This figure shows that the test portrayed is both unbiased and consistent.}
	\label{fig:hypothesis_testing_power}
\end{figure}
...
\todo{Example of the two peaks where we need to actually do a GOF not hypothesis test: I leave this to M.L (if he wants) as it was an instructive example when I first worked it out (and because I want to go on to new things at this moment!)}
