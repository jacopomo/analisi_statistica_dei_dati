...
However, for a probability to represent a stochastic process rather than a deterministic law, the limit must be invariant under whichever way we choose a subsequence of trials, provided our choice does not depend on the outcome of those trials (\emph{place selection invariance}). If the limit changes when we sample a subsequence based on prior outcomes, the sequence is predictable, and a frequentist probability cannot be defined.

\begin{example}
	In the previous example of extracting colored balls with replacement, I obtain the following sequence:
	
	\[
	\text{red, blue, green, red, blue, green, red, blue, green, red, }\dots
	\]
	
	In this sequence, $P(\text{red})$ is not well defined, as if I consider all of the events in the limit I get $P(\text{red})=\frac{1}{3}$, but if I consider all of the events that follow the extraction of a green ball, $P(\text{red})=1$ and all those following a blue extraction $P(\text{red})=0$. These three limits were all taken in the mathematical sense to infinity, though they are not equal to the same value and therefore the limit is not well defined, and in fact this deterministic sequence does not have a probability defined. 
\end{example}

This is precisely what is meant by the assertion that frequentist probability is defined on an ensemble, for the Von Mises definition to hold we must be able to, at least conceptually, repeat identical experiments to infinity. Thus, without a clearly defined ensemble to establish what constitutes an "identical trial," the limit $N \to \infty$ lacks a proper sample space and remains mathematically ambiguous.
...
These three axioms will serve as the backbone for the following definitions and theorems:
...
It's not hard to see that $P(A|B)$ (also denoted with $P_B(A)$) is a Kolmogorov probability and satisfies all of the necessary axioms.

It is important to note that conditional probability has nothing to do with causation, correlation, or temporal relations between two events. Simply put, it is the probability of an event given the fact that another has already taken place, and therefore is nothing more than a restriction of the probability space to a smaller ensemble. 
...
This theorem is useful when the total probability is not readily calculated without considering the single, conditional cases.
\begin{example}
...
\begin{example}
	...
	Now Monty wants to spice things up for the audience. After your choice, he decides to open a door, differently than the one you chose. He opens door number three, revealing a goat (to be clear: he cannot open the door you chose, nor the one hiding the car, revealing the prize immediately would not make for a great show, would it?).
...
The Bayesian school uses a concept of probability fundamentally different from the frequentist (von Mises) framework. To try to minimize the natural confusion that will follow this chapter, we will baptize this concept with a different name: \emph{"degree of belief"} $\pi(x)$. 
Under this subjective paradigm, probability represents a measure of plausibility given a specific state of knowledge, rather than a physical property of an infinite sequence. 
Unlike the frequentist definition, which strictly requires an infinitely replicable experiment, a degree of belief can be assigned to any proposition, even unique, unrepeatable events. Crucially, this allows us to assign degrees of belief directly to constant, non-observable model parameters\sidenote{In the frequentist sense there is no such thing as the probability of a parameter.} (such as the true mass of a particle), treating our uncertainty about nature as a probability distribution.
Furthermore, while a frequentist uses data to compute an objective limiting frequency, a Bayesian treats probability as dynamic. They start with an initial state of knowledge (the \emph{prior} belief) and use experimental data as evidence to update it into a sharper, modified state of knowledge (the \emph{posterior} belief).

Despite these philosophical departures, the Bayesian degree of belief behaves perfectly under standard mathematical laws and has been shown to satisfy the Kolmogorov axioms. 

Bayesian probability will follow us throughout the rest of this text, and often, when applicable, will be the first interpretation used to explain difficult concepts, thanks to its easy interpretation and possibility of assigning degrees of belief directly to non-observable parameters. For now, the crucial takeaway is to avoid mixing these two entirely separate definitions of the word "probability."
...
Since we have defined pdf's starting from integrals, we can see that they adopt the mathematical properties that we are familiar with. For example, with the same integrands the sum of two integrals with different bounds is the integral over the total bound, which naturally means that if the integral of the pdf is $1$ over the full space then the integral of each of its parts will be less than $1$.

Because of this integral definition it also follows that the sum and product of two pdf's is not universally the pdf of the sum or product of the random variables themselves. 

We use the term "function", but if we intend the integral in the Lebesgue sense we can extend the definition to pdf's that are not functions but rather distributions. In general, this allows us a series of liberties that functions formally would not allow, such as mixing discrete and continuous variables. It's not guaranteed that a pdf satisfies the Kolmogorov axioms, and the following example shows that while $p(x)\geq 0$ not necessarily $p(x) < 1$.
...
The fundamental theorem of calculus also tells us that $\dv{F}{x}=p(x)$. We will find the cumulative easier to calculate than the density function itself in many cases. Naturally, if the domain of $x$ is restricted, then the integral is defined from its lowest value until $x$ itself, instead of starting from $-\infty$.
...
Before diving deeply into the Law of Large Numbers, it's customary to see a certain theorem that will be useful in its proof.
...
\begin{lemma}[Čebyšëv's theorem]
	...
	\begin{equation}
		...
		\label{eq:chebyshev}
	\end{equation}
	exists for each $k\in\R^+$. This provides a conservative and easy to calculate upper bound for the probability of finding a point a certain amount of standard deviations from the mean. 
\end{lemma}
...
\begin{example}[Proof of Čebyšëv's theorem]
	...
	\begin{figure}[H]
		...
		\caption{Visualization of the indicator function that defines Čebyšëv's inequality. Its construction is such that its value is null for all $x$ that are less than one $\sigma$ from the mean $\mu$, and unitary everywhere else.}
	\end{figure}
...
Now we can get to stating and proving the LLN, armed with the Čebyšëv inequality. 
...
Within this theorem we have defined an interesting property "under the rug", which is convergence in probability, which is nothing more than the limit in (\ref{eq:LLN})

If we had to write the law of large numbers in words, it would read: "the sample mean of an observable converges in probability to its expected value whenever the latter exists and is finite". For those of you who are not on their first read of these notes, or have some prior knowledge on point estimators, we can say that "the arithmetic mean is a consistent (point) estimator for the mean $\E[x]$ of the distribution." Don't worry if you don't know what that means right now, we'll talk about point estimators extensively in the future.
...
The most general proof of the LLN is beyond the scope of this course and therefore this write-up, however we will look at a more limiting but still instructive example. Let's suppose that also the following integral exists (\emph{variance of} $X$):
	...
	We notice that the pdf of the vectorial observable $\{x_i\}_N$ can be written as the product of the \emph{pdf's} of the single observables, assuming that the observations are all independent: $p(x_N)=\prod_i^N p_X(x_i)$. Therefore, we have that
...
As we have seen, knowing the moments of a distribution is a powerful tool that has numerous implications, though actually calculating them can be non trivial. For example, let's take the case where we have a non-linear function $f(\vec{x})$ of some vector of observables $\vec{x}$, and we happen to know that $\E[\vec{x}]=\vec\mu$. We want to try to find the expected value (well, any moment really), of the function $f(x)$. What we can do is expand using Taylor around the mean, as we can imagine that $f(\vec{x}) \simeq f(\vec\mu) +$ corrections. Doing so:

	\[
	f(\vec{x}) \simeq f(\vec{\mu}) + \frac{\partial f}{\partial x_i}\bigg\vert_{x_i = \mu_i}(x_i-\mu_i)+\frac{1}{2}\frac{\partial^2f}{\partial x_i \partial x_j}\bigg\vert_{x_{i,j} = \mu_{i,j}}(x_i-\mu_i)(x_j-\mu_j)
	\]
	
We can now take the expected value of both sides, as it is a linear function:
	\[
	\E[f(\vec{x})] \simeq \E[f(\vec{\mu})] + \frac{\partial f}{\partial x_i}\cancelto{0}{\E[x_i-\mu_i]} \quad + \frac{1}{2}\frac{\partial^2f}{\partial x_i \partial x_j}\E[(x_i-\mu_i)(x_j-\mu_j)]
	\]

Where in the linear term we have identified the expected value of each $x_i$ subtracted from their respective means, which is null by definition. We have also not explicitly written that the derivatives are to be evaluated in the respective means for clarity. What this leaves us with is:

	\[
	\E[f(\vec{x})] \simeq f(\vec{\mu}) + \frac{1}{2}\frac{\partial^2f}{\partial x_i \partial x_j}\operatorname{cov}[x_i,x_j]
	\]

Which in the case of a one-dimensional observable simplifies to:

	\[
	\E[f(x)] \simeq f(\mu) + \frac{1}{2}f^{\prime\prime}\Var[x]
	\]

In this way we have approximately calculated the expected value of a non-linear function of the observable, without having to explicitly change variables and integrate. This form is also useful if we take it a step further. Going back to the multi-dimensional case, we now consider the linear (first order) approximation (where $\E[f(\vec{x})]\approx f(\vec{\mu})$ and $f(\vec{x}) \approx f(\vec{\mu}) + \frac{\partial f}{\partial x_i}(x_i-\mu_i)$). In this regime, we have:
...
Which for uncorrelated $x$ (diagonal covariance matrix) simplifies to:
	\[
	\sigma_f^2 = \Var[f(\vec{x})] = \sum_i \left| \frac{\partial f}{\partial x_i} \right|^2\sigma_{x_i}^2 +\dots
	\]
	
We recognize this as our familiar \textbf{formula for the propagation of errors}. Interesting to note that this formula comes from a Taylor approximation.

Essentially, using the moments we can approximate the expected value and variance of non-linear functions of the (in this case uncorrelated) observables, with:

\begin{align*}
\E[f(\vec{x})] &\simeq f(\vec{\mu}) + \frac{1}{2}\frac{\partial^2f}{\partial x_i \partial x_j}\Var[x_i] \\
\Var[f(\vec{x})] &\simeq \sum_i \left| \frac{\partial f}{\partial x_i} \right|^2\sigma_{x_i}^2
\end{align*}

Where the derivatives in $x_i$ and $x_j$ are evaluated in $\mu_i$ and $\mu_j$, respectively.
...
\begin{example}[Proof of the central limit theorem]
	...
	\begin{align*}
		...
		\simeq& N\log\left[ 1 + \frac{\partial\phi_y}{\partial t}\bigg\vert_{t=0}\frac{t}{\sqrt{N}} + \frac{\partial^2\phi_y}{\partial t^2}\bigg\vert_{t=0}\frac{t^2}{2N} + o\left(\frac{t}{\sqrt{N}}\right)^2\right]\\
...
\end{example}
\sidenotetext{Whatever that means}
