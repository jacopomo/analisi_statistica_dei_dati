...
\begin{definition}[Probability density function \emph{pdf}]
...
An interesting and somewhat unexpected result can be reached when talking about \emph{CDF's}: the \textbf{probability integral transform} \cite{david1948probability} (also known as the \emph{universality of the uniform}\sidenote{If you haven't done so already, take a moment to brush up on what a Uniform distribution is, by heading over to the appendix \ref{Appendix:discrete}}).
...
\begin{align*}
	F_Y(y) &= P\left(Y\leq y\right) = P\left(F_X(X)\leq y\right) = P\left(X\leq F_X^{-1}(y)\right)\\
...
\begin{definition}[Mean]
	If $p_X(x)$ is the \emph{pdf} of $X$, we call the \emph{mean} of the distribution (or the expected value of $X$) the integral (Lebesgue):
	\[
...
\begin{theorem}[Law of large numbers (LLN)]
	Let $X$ be a real-valued observable, and $p(x)$ its \emph{pdf}. Let's suppose that the mean of $X$ exists and is finite:
	\begin{equation}
...
Now we can get to proving the LLN, armed with the Čebyšëv inequality. We notice that the \emph{pdf} of the vectorial observable $\{x_i\}_N$ can be written as the product of the \emph{pdf's} of the single observables, assuming that the observations are all independent: $p(x_N)=\prod_i^N p_X(x_i)$. 
...
Applying the Čebyšëv inequality (\ref{eq:chebyshev}) to the statistic $\bar{x}_N$, with $k=\varepsilon/\sqrt{\Var[{\bar{x}_N}]}$ we get:
	\[
...
\subsubsection{Probability distribution of a statistic}
Let's begin with the most pressing one: how do we define the \emph{distribution} of a \emph{statistic}? Well, if I know the discrete \emph{pmf} of $x$ then the \emph{pmf} of the statistic $f(x)$ is easily written as $pmf\left(x(f)\right)$, no surprises there. 
...
\subsubsection{Expected value of a statistic}
...
$\E_X$ is a linear operator, and for the LLN $\E_X[x]=\mu$. We also take the opportunity to introduce two helpful properties:
	\[
	\E_x\left[ (x-\mu)^2 \right] = \E_X[x^2] - \mu^2 = \Var[X]
	\]
	\[
...
\subsubsection{Moments of a statistic}
...
\begin{theorem}[Central limit theorem]\label{th:central_limit}
	Let $x$ be a real observable with a distribution $p(x)$ that admits the first two moments. 
	Then, the distribution of the sample mean 
	\[
	\bar{x} = \frac{1}{N} \sum_{i=1}^{N} x_i
	\]
	in the limit of large $N$ tends to a distribution of universal form, independent of the specific shape of $p(x)$.
\end{theorem}

\begin{example}[Proof of the central limit theorem]
	Let's begin by considering the transformation
	\[
	y = x-\mu; \quad y_n = \sqrt{N}\cdot\bar{y}_n = \sum_i\frac{y_i}{\sqrt{N}}
	\]
	We note that already we have $\E[y_n]=0$ and $\Var[y_n]=\sigma^2$ independently of $n$, as the variance is additive and goes with the square of any multiplicative constant. Using the properties of the characteristic function we have just seen, we define:
	\[
	\phi_n(t)\equiv \phi_{y_n}(t) = \phi_y^N\left(\frac{t}{\sqrt{N}} \right)
	\]
	from which we write
	\begin{align*}
		\log\left(\phi_n(t)\right) &= N\log\left[ \phi_y \left(\frac{t}{\sqrt{N}}\right) \right] \simeq N\log\left[ 1 + \frac{\partial\phi_y}{\partial t}\frac{t}{\sqrt{N}} + \frac{\partial^2\phi_y}{\partial t^2}\frac{t^2}{2N} + o\left(\frac{t}{\sqrt{N}}\right)^2\right]\\
		&=N\log\left[1+0-\frac{\sigma^2t^2}{2N}+ o\left(\frac{t}{\sqrt{N}}\right)^2\right]\simeq N \left(-\frac{\sigma^2t^2}{2N}+ o\left(\frac{t}{\sqrt{N}}\right)^2\right)\\
		&=-\frac{\sigma^2t^2}{2}+ N\cdot o\left(\frac{t^2}{N}\right)\Rightarrow\log\left(\phi_n(t)\right)\rightarrow\log\phi(t)=-\frac{\sigma^2t^2}{2}\\
		&\Rightarrow \phi(t) = \lim_{n\to\infty}\phi_n(t)=\exp\left(-\frac{\sigma^2t^2}{2}\right)
	\end{align*}
	
	This $\phi(t)$ is universal, and does not depend on $p(x)$ \qed
...
We now want to recover the shape of this particular distribution whose characteristic function is $\phi(t)$. In other words, we want to perform a reverse Fourier transform to recover the explicit form of the limit distribution.
	...
	\begin{align*}
	g(x)&=\frac{1}{2 \pi}\int_{-\infty}^{\infty}\exp\left(\frac{-\sigma^2t^2}{2}dt\right)\exp\left(-itx\right) dt\\
	&=\frac{1}{2 \pi}\int_{-\infty}^{\infty}\exp\left(\frac{-\sigma^2t^2-2itx-i^2x^2/ \sigma^2+i^2x^2/ \sigma^2}{2}\right) dt\\
	&=\frac{1}{2 \pi}\int_{-\infty}^{\infty}\exp\left(-\frac{\left(\sigma t -ix/\sigma \right)^2}{2}\right)\exp\left(-\frac{x^2}{2\sigma^2}\right) dt\\
	&=\frac{\exp\left(-\frac{x^2}{2\sigma^2}\right)}{2 \pi}\int_{-\infty}^{\infty}\exp\left(-\frac{\left(\sigma t -ix/\sigma \right)^2}{2}\right)dt\\
	\end{align*}\todo{The undersized integral symbols really bother me :(, we should try to fix this in the future}

The integrand is a holomorphic function with no poles\sidenote{If complex analysis isn't your strong suit, don't worry, you can kind of just follow along} thus we can perform the substitution $z=\sigma t -ix/\sigma$:

	\begin{align*}
	g(x)&=\frac{\exp\left(-\frac{x^2}{2\sigma^2}\right)}{2 \pi\sigma}\int_{-\infty}^{\infty}\exp\left(-\frac{z^2}{2}\right)dz\\
	&=\frac{1}{\sqrt{2\pi}\sigma}\exp\left(-\frac{x^2}{2\sigma^2}\right) \equiv \N(x; 0, \sigma^2)
	\end{align*}

This is the Gaussian (or normal) \emph{pdf}, and the way we derived it highlights its fundamental importance in statistics. It is important to emphasize that the shape of this limit distribution does not depend on the original distribution of $x$.\\
