...
With a solid understanding of $p$-values under our belt, it's time to move on to the practical applications of this theory. Let's begin with the case where the most general hypothesized pdf can be described as a member of a parametrized space $p(x;\theta, \nu)$. This space can even have a high number of dimensions, and $H_0$ is the case of $\theta=\theta_0$. This means that, since we consider the parameters $\theta$ to be the ones fixed by the hypothesis, the parameters $\nu$ can be considered nuisance parameters.
...
Conversely, if the null hypothesis is \textit{“the data follows a Gaussian distribution with mean $\mu_0$”}, then one parameter is fixed by the hypothesis, namely the mean. In this case, the dimension of $\theta$ is one, while the variance is treated as a nuisance parameter.
...
Considering a sequence of experimental measurements of a real observable $x$, let $k_i$ be the number of events $\{x_j\}$ falling into the bin $i\in \left[1,\dots,n\right]$. Therefore, what we have is a histogram of $n$ bins, $N$ measurements, completely defined by $\{k_i\}_n$ such that $\sum_i^n k_i =N$.

As we have already discussed, each bin count is independent and follows a Poisson distribution, such that:

	\[
	p(x;\theta, \nu) = \prod_i^n \mathcal{P}\left(k_i,\mu_i(\theta,\nu)\right)
	\]
	
where we know $\mu_i(\theta,\nu)=\E\left[k_i\right]$ to be the expected number of events in the $i$-th bin. We'll come back later on what $\mu_i$ represent, for the following calculations we don't need further information.


Our goal here is to confirm that the observed distribution does not statistically differ from the hypothesized pdf, essentially fitting our data to the model we assume. Adding \emph{enough} parameters, that we might call $\xi$, to our current pdf, then we can generally obtain whichever vector of expected values $\mu_i(\theta,\nu)$. In a certain sense, we can't explicit the most general alternative hypothesis, but proceeding in this way we can convince ourselves that we could arbitrarily "fit any histogram" given enough parameters.

In this case, the LLR becomes:

	\[
	\lambda(x) = 2\ln\frac{\sup_{\theta,\nu, \xi}\prod_i\mathcal{P}\left(k_i;\mu_i(\theta,\nu,\xi)\right)}{\sup_\nu\prod_i\mathcal{P}\left(k_i;\mu_i(\theta_0,\nu)\right)}
	\]

To evaluate this, let's look at the numerator. The term $\sup_{\theta,\nu,\xi}\mu_i(\theta,\nu,\xi)$ represents the mean of the Poisson distribution in the $i$-th bin under the most general possible hypothesis. If we assume this general model has enough parameters to "fit any histogram" to any degree of precision, it becomes what is known as the \emph{Saturated Model}. 

In this saturated case, the Likelihood is maximized when each bin's expected value is exactly equal to its observed count, $\hat{\mu}_{i} = k_i$. Essentially, we are allowing each of the $n$ bins to behave as an independent free parameter. Conversely, the denominator represents our specific physical model, which constrains those $n$ bins through fixing $\theta_0$ and using only a small set of free parameters $\nu$.

Following Wilks' theorem, the degrees of freedom for our test statistic will be the difference in the number of free parameters between these two models. Say the Saturated Model uses $m$ parameters\sidenotemark\sidenotetext{Consider the case of $N$ observations distributed among $n$ bins, with $N$ fixed. The saturated model treats the expected count in each bin as an independent parameter, except for one parameter that is constrained by the normalization condition $\sum_i k_i = N$.
	
As a consequence, there are not $n$ independent parameters, but rather $m=n-1$. You have probably encountered this result at least once in your life when fitting histograms}.

We know that our model uses $\dim(\nu)$ free parameters, thus:
	\[
	DOF = m - \dim(\nu)
	\]
By evaluating the Poisson product at these respective maxima, which are $\hat{\mu}_{i} = k_i$ for the numerator and $\mu_i(\theta_0, \hat\nu)$ for the denominator (consider the "hats" to be the \emph{MLE}'s, not explicitly stated for the sake of clarity), our expression becomes:

	\[
	\lambda(x) = 2\ln\frac{\prod_i^n \mathcal{P}\left(k_i;k_i\right)}{\prod_i^n \mathcal{P}\left(k_i;\mu_i(\theta_0,\hat{\nu})\right)}
	\]

We expand using the definition of a Poisson statistic (we use $\hat{\mu_i}\equiv\mu_i(\theta_0,\hat{\nu})$ to simplify the notation during the calculation, returning to the full expression in the results):
...
It is now time to address the questions about the nature of $\mu_i$, or, better, $\mu_i(\theta_0, \hat\nu)$. This represents te expect number of events in the $i$-th bin:
\[
	\mu_i(\theta_0, \hat\nu)=\E[k_i(\theta_0, \hat\nu)]=\E[\mu\mathfrak{p}_i(\theta_0, \hat\nu)]=\mathfrak{p}_i(\theta_0, \hat\nu)\E[\mu]=N\mathfrak{p}_i(\theta_0, \hat\nu)
\]
where $\mu$ is the expected total number of events (and $\E[\mu]=N$ indeed) and $\mathfrak{p}_i(\theta_0, \hat\nu)$ represents the probability of an observation to fall into the $i$-th bin. This result is what we expect: the expected number of events falling into the $i$-th bin is the total number of observed events multiplied by the probability of falling into the $i$-th bin.

We only need to understand who $\mathfrak{p}$ is. We can easily find that:

\[
	\mathfrak{p}_i(\theta_0, \hat\nu)=
	\begin{cases}
		\int_{S_i}p(x;\theta_0, \hat\nu)\;dx \quad &\text{if} \; p(x;\theta_0, \hat\nu) \; \text{continuous}\\
		P(x_i;\theta_0, \hat\nu) \quad &\text{if} \; p(x;\theta_0, \hat\nu) \; \text{discrete}		
	\end{cases}
\]
where $\int_{S_i}$ is to be intend as the integral over the "volume" of the $i$-th bin and $P(x_i;\theta_0, \hat\nu)$ the probability of observing $x_i$ corresponding to the $i$-th bin.

Hopefully the next examples will clear things out.

\begin{example}
	Multinomial die distribution example
\end{example}
...
\begin{example}
	...
	We would then compare this value to a K-S critical values table such as \ref{tab:ks_table_values} for $N=10$. For a significance level of $\alpha = 0.05$, the critical value is approximately $0.41$, which we fall below and therefore fail to reject $H_0$ at this level of significance.\sidenotemark
...
\hrule
\begin{center}
	\textbf{Exercises at: \ref{exer:gof}}
\end{center}
\hrule