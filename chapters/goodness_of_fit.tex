...
For example, if the null hypothesis is \textit{“the data follow a Gaussian distribution”}, then no parameter is fixed under $H_0$. Both the mean and the variance are therefore treated as nuisance parameters~$\nu$.
Conversely, if the null hypothesis is \textit{“the data follow a Gaussian distribution with mean $\mu_0$”}, then one parameter is fixed by the hypothesis, namely the mean. In this case, the dimension of $\theta$ is one, while the variance is treated as a nuisance parameter.
