...
While we have recently learned that for continuos distributions we can't write a general GOF test, we can construct one that is "pretty good" in the case of a one-dimensional real-valued observable $X\in\R$.

Let $H_0$ be the simple hypothesis that the cumulative of $X$ be a certain $F(x)$, and that this cumulative be completely known. We then construct the statistic:
...
We are essentially finding the max deviation, over the observed data, between the cumulative of the data and the "sample cumulative", defined as:

	\[
	S_i = \sum_{j:x_j<x_i}\frac{1}{N}
	\]

[can we describe this real quick? "The sample cumulative is essentially..."]
The critical region for this test is $D_N>q_\alpha$, the threshold for which can be found in tables. 

We can see that the distribution of $D_N$ assumes higher values under $\cancel{H_0}$ than $H_0$, making it an unbiased right-tailed test. We can also see that $\lim_{N \to \infty} D_N = 0$ under $H_0$ but not if it is rejected, which also makes it a consistent Test. Furthermore, the \emph{pdf} $p(D)$ does not depend on $F(x)$ [WHY??]


These properties allow us to establish $D_N$ as a good Test statistic, and especially its independence from $F(x)$ allows us to create tables for the thresholds of $D$ for each $N$, on which the statistic depends.
