...
\begin{figure}[H]
	...
	\caption{Distribution of the $p$-value statistic under the null hypothesis $H_0$ and an example alternative $H_1$. A vertical line corresponding to $\alpha=0.05$ significance level is also shown.}
...
We can also visualize the different types of errors we may commit: obtaining $p<\alpha$ suggests that it was not extracted from $H_0$, though naturally there is $\alpha$ chance for it to come from the uniform distribution, and so a probability of $\alpha$ to have committed a type one error, and erroneously rejected the null hypothesis. Hopefully this example clears the question of why we can attribute to the $p$-value the probability under $H_0$ to have a more extreme solution than what was observed.
...
To find the threshold $q>q_alpha$, we consider the fact that $-2\ln\left(p_i\right)$ has an exponential distribution and that:

	\[
	\chi^2_\nu \sim \frac{1}{2^{\frac{\nu}{2}}\Gamma\left(\frac{\nu}{2}\right)}x^{\frac{\nu}{2}}e^{-\frac{x}{2}} \Rightarrow \chi_2^2 \sim \frac{1}{2}e^{-\frac{1}{2}} \Rightarrow -2\ln\left(p_i\right) \sim \chi_2^2
	\]

We therefore find the threshold $q>q_\alpha$ considering that $a\sim\chi^2_{2n}$. 
