...
Following Wilks' theorem, the degrees of freedom for our test statistic will be the difference in the number of free parameters between these two models. Say the Saturated Model uses $m$ parameters\sidenotemark\sidenotetext{Consider the case of $N$ observations distributed among $n$ bins, with $N$ fixed. The saturated model treats the expected count in each bin as an independent parameter, except for one parameter that is constrained by the normalization condition
	\[
	\sum_i k_i = N 
	\]
