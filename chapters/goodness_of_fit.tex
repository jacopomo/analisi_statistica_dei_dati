...
So far, we have seen the Likelihood Ratio GOF test for binned sampled data (histograms). We now ask whether it is possible to generalize this theory to the unbinned case. Specifically, whether a general GOF test exists for \emph{pdf}'s of real-valued observable data. 
...
\begin{example}
	...
	With \emph{MLE} $\hat{m_{\text{MLE}}} = \max\left(x_i\right)$. Suppose we have two different toy datasets:
	...
	The maximum Likelihood only considers the value of the \emph{pdf} at the points where data exists. It is completely blind to the global distribution of the data, ignoring the fact that dataset $1$ is a much better fit than dataset $2$ for a uniform distribution. 
